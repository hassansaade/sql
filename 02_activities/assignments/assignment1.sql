...
SELECT 
	...
	,CASE 
		...
		END as prod_qty_type_condensed
	,CASE 
		WHEN LOWER(product_name) LIKE LOWER('%Peppers%')
		THEN 1
		ELSE 0
		END as pepper_flag
FROM product;
...
SELECT v.vendor_name, count(vba.booth_number) as booth_count
FROM vendor v
INNER JOIN vendor_booth_assignments vba
	ON v.vendor_id = vba.vendor_id
...
SELECT c.customer_last_name, c.customer_first_name, sum(cp.quantity*cp.cost_to_customer_per_qty) as amount_spent
FROM customer c
